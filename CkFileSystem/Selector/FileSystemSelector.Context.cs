...
public partial class CkFileSystemSelector<T, TStateStorage>
{
    ...
    private void RightClickContext(CkFileSystem<T>.Folder folder)
    {
        ...
        foreach ((Action<CkFileSystem<T>.Folder>, int) action in _rightClickOptionsFolder)
    ...
    private void RightClickContext(CkFileSystem<T>.Leaf leaf)
    {
        ...
        foreach ((Action<CkFileSystem<T>.Leaf>, int) action in _rightClickOptionsLeaf)
    ...
    {
        foreach ((Action, int) action in _rightClickOptionsMain)
            action.Item1.Invoke();
    }
    ...
    {
        string currentPath = folder.FullName();
